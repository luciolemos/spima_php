...
Class Locale {
    ...
    static public function setApplicationLanguage() {
        ...
        if (CFG_MULTI_LANG_ENABLED) {
            ...
            if (isset($langInRequest)) {
                ...
                // Otherwise, browser language is read from the HTTP request
                $language = substr(\Request::getAcceptLanguage(), 0, 2);
            }
        }
    ...
    static public function setLocale() {
        ...
        $currentLocale = call_user_func_array('setlocale', $localeSettings);
    ...
    static public function getThousandsSeparator() {
        if (LC_LOCALE_THOUSANDS_SEPARATOR === NULL) {
            $locale = localeconv();
            return \Convert::toUTF8($locale['mon_thousands_sep']);
        } else {
    ...
    static public function getCurrencySymbol($forPdfPrinting = FALSE) {
        if (LC_LOCALE_CURRENCY_SYMBOL === NULL) {
            $locale = localeconv();
            $symbol = \Convert::toUTF8($locale['currency_symbol']);
    ...
    /**
     * Returns the date format used by the strftime() PHP function according
     * the current selected language
     * @return string Date format matching the strftime() PHP function format
     */
    static public function getLocaleDateFormat() {
        if (LC_LOCALE_DATE_FORMAT === NULL) {
            return '%x';
        } else {
    ...
    static private function includeApplicationTranslations(&$language) {
        ...
        if (!include "app/lang/locale_" . $language . ".php") {
            $language = CFG_DEFAULT_LANGUAGE;
            if (!include "app/lang/locale_" . $language . ".php") {
                include "app/lang/locale.php";
            }
        }
    ...
    static private function includeCoreTranslations($language) {
        \ErrorHandler::suspend();
        if (!include "lang/locale_" . $language . ".php") {
            include "lang/locale_en.php"; // English by default if $language not supported 
        }
        \ErrorHandler::restart();
    }
    ...
    static private function includeModulesTranslations($language) {
        ...
        if ($modules) {
            foreach ($modules as $moduleName) {
                $path = $moduleName . '/mod/lang/locale_%1.php';
