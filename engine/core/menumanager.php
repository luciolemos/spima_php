...
Class MenuManager {
    ...
    static public function getMenuItemFromURI() {
        if (\Parameters::isSetPageReload() && \Request::getController() === 'httperror') {
            $redirectURL = str_replace(\General::getAbsoluteURI(), '', filter_input(INPUT_SERVER, 'REDIRECT_URL', FILTER_SANITIZE_URL)); // Base URI removed
            $URLpieces = explode('/', $redirectURL);
            if (count($URLpieces) === 1) {
                return self::getMenuItemByCriteria(3, $URLpieces[0]); // SEO link is used to find the menu item
            } else {
                return NULL;
            }
        } else {
