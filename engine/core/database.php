...
Class Database {
     ...
     */
    static private function connectToDb($host, $database, $user, $password) {
    ...
    static private function getDbConnection($level = 'appl') {
        ...
        if (!$dbConnection) {
            $dbConnection = self::connectToDb(CFG_SQL_HOST, $dbName, $user, $password);
     ...
     * @return \PDO PDO object of the connection
     */
    static public function getCustomDbConnection($host, $database, $user, $password) {
        return self::connectToDb($host, $database, $user, $password);
    }
